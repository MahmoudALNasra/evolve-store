...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (argv[i] === '--include-unpublished') args.includeUnpublished = true
  }
  return args
}
...
async function main() {
  ...
  const before = await auditDescriptionQuality({ includeUnpublished: args.includeUnpublished })
  ...
  const mode = args.needsWork ? 'needs-work' : args.onlyMissing ? 'only-missing' : 'all'
  console.log(`\nOptimizing descriptions (${mode}): limit=${args.limit} skip=${args.skip} dryRun=${args.dryRun} all=${args.all} includeUnpublished=${args.includeUnpublished}`)
  ...
  const after = await auditDescriptionQuality({ includeUnpublished: args.includeUnpublished })
  console.log('\n--- Finished ---')
